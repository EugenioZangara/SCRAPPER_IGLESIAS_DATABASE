...
Campos de ubicación geopolítica (después de `barrio`):
- `provincia`: CharField(100), default "Ciudad Autónoma de Buenos Aires"
- `ciudad`: CharField(100), localidad/ciudad
- `diocesis`: CharField(150), ej: "Arquidiócesis de Buenos Aires"
- Todas las parroquias existentes tienen estos campos poblados via migración 0026.
- `publico_buscar` filtra por `provincia` (param GET `provincia`).
- `publico_inicio` expone `provincias` y `total_provincias` al contexto para el selector.
...
El proyecto tiene dos sistemas de diseño distintos según el contexto:

---

### Vistas públicas — Sanctuary Design System
Aplicado a todos los templates bajo `apps/iglesias/templates/iglesias/publico/`.
Tono: profesional, comunitario, accesible. Apropiado para usuarios finales en mobile.

#### Fuentes
- Inter (400, 500, 600) — todo el sistema, desde Google Fonts

#### Variables CSS principales (`base_publica.html`)
```
--primary: #2a4494       ← azul profundo (confianza y tradición)
--primary-soft: #e8edf8  ← fondo azul suave (badges, selección)
--primary-dark: #1e3270  ← azul oscuro (hover, texto sobre primario)
--ink: #1a2a5c           ← texto principal
--muted: #6b7280         ← texto secundario
--rule: #e5e7eb          ← bordes
--rule-2: #f3f4f6        ← fondo secciones
--paper: #ffffff         ← fondo cards
--gold: #FFD700          ← dorado espiritual (puntos, verificación, gamificación)
--gold-soft: #FFF9CC     ← fondo dorado suave
--gold-dark: #a07b00     ← texto sobre fondo dorado
```

#### Uso del dorado
- Números de score en `ranking.html` y `detalle.html`
- Badge de puntos del usuario (fondo `--gold-soft`, texto `--gold-dark`, borde `--gold`)
- No usar en texto sobre fondo blanco (contraste insuficiente)

#### Colores heredados de redes sociales (no modificar)
- Instagram: bg `#fce7f3`, icon `#9d174d`
- Facebook: bg `#eff6ff`, icon `#1e40af`
- YouTube: bg `#fef2f2`, icon `#dc2626`
- TikTok: bg `#f0fdf4`, icon `#166534`

---

### Panel admin/interno — Editorial Design
Aplicado a `apps/iglesias/templates/iglesias/base.html` y vistas internas.
Tono: editorial / archivo, apropiado para staff.

#### Fuentes (Google Fonts)
- Newsreader — serif display (títulos, valores grandes)
- Geist — sans neutra (cuerpo, nav, botones)
- JetBrains Mono — mono (labels, metadatos, IDs)

#### Variables CSS principales (`base.html`)
```
--primary: #4f46e5       ← índigo (panel admin)
--paper: #ffffff
--ink: #1e1b4b
--accent: #4f46e5
```
...
- Template Django admin: `apps/iglesias/templates/iglesias/detalle_parroquia.html`
- Estilos base admin: `apps/iglesias/templates/iglesias/base.html`
